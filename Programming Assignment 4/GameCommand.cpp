...
{
    Pokemon* P = (model).GetPokemonPtr(pokemon_id);
    if (P != NULL && (model.GetBattleArenaPtr(arena_id) != NULL)){
        cout << "Moving " << P->getName() << " to BattleArena " << model.GetBattleArenaPtr(arena_id) ->GetId() << endl;
        P -> StartMovingToArena(model.GetBattleArenaPtr(arena_id));
    }
    else{
        cout << "Error: Please enter a valid command!" << endl;
    }
...
void DoBattleInArenaCommand(Model& model, int pokemon_id, int rival_id){
    Pokemon* P = (model).GetPokemonPtr(pokemon_id);
    if (P != NULL){
        cout << P->getName() << " is battling against " << model.GetRivalPtr(rival_id)->getName() << endl;
        if(P->GetCurrentArena() != NULL){
            P->ReadyBattle(model.GetRivalPtr(rival_id));
            P->StartBattle();
        }
        else{
            cout << "Error: Not in an Arena!" << endl;
            return;
        }
    }
    else{
...
void DoGoCommand(Model& model, View& view){
 ...
 void CreateNewObjectCommand(Model& model, char c, int id, double x, double y){
     model.CreateNewObject(c,id,x,y);
 }
